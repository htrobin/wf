...
$(function() {
    if (gIsNewDoc && gWFID == "") {
        ...
        //读取流程图
        var Path = "/" + gCurDB + "/vwWFXML/" + gCurDocID + "?OpenDocument";
        if (gIsNewDoc) {
            Path = "/" + gWorkFlowDB + "/vwWFXML/" + gWFID + "?OpenDocument";
        }
        $.ajax({
            url: Path,
            cache: false,
...
function initOnLoad(arrTmpCurUser) {
    ...
    $.each(gArrBtns,
    function(i, e) {
        $(e.align == "1" ? $(btndom).appendTo("#btnContL") : $(btndom).appendTo("#btnContR")).attr({
            text: e.name,
            title: e.title,
            id: (e.id ? e.id: ""),
            iconCls: e.ico,
	...
	$("[id$=_att]").each(function(i, c) {
		if ($(c).attr("toolbar") != undefined) {
			...
			$("<div class='mini-toolbar toolbar'><div class='toolbar-title'><span class='mini-button-text mini-button-icon icon-fj'></span>"+t.title+"</div>"
			 ...
			 +"<a class='mini-button "+t.upId+"' plain='true' id='"+t.upId+"' iconCls='icon-upload' href=\"javascript:goUploadFile('"+$(c).attr("id")+"');\" >\u4e0a\u4f20</a>"
			 +"<a class='mini-button "+t.editId+"' plain='true' id='"+t.editId+"' iconCls='icon-edit' href=\"javascript:goEditAttachFile('"+$(c).attr("id")+"');\" >\u7f16\u8f91</a>"
			 +"<a class='mini-button "+t.delId+"' plain='true' id='"+t.delId+"' iconCls='icon-remove' href=\"javascript:goDelAttachFile('"+$(c).attr("id")+"');\" >\u5220\u9664</a>"
...
function attachSelectAll(obj) {
    $.each($('input[name="$$attachfile"]', "#AttachFileGrid"),
    function(i, item) {
        item.setAttribute("checked", obj.checked);
    });
}
function goEditSendFile() {
    ...
    window.open(encodeURI(strPathUrl), "", pstatus);
}
